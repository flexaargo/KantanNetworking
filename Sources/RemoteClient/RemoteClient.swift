...
public protocol RemoteClient {
    ...
    /// Makes a request to the configured host using information from the ``Routable``. Decodes a
    /// `Response` from the the data payload. This method will throw an error if the HTTP response
    /// is not OK or if there was an error decoding the `Response`
    /// - Parameters:
    ...
    func request<Response>(_ responseType: Response.Type, from route: Routable) async throws
        -> Response where Response: Decodable
